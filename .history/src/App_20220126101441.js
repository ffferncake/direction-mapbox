    ...
    async function getData() {
      ...
      map.current.on("idle", () => {
        map.current.loadImage(
          "https://docs.mapbox.com/help/demos/using-recolorable-images-in-mapbox-maps/shop-15.png",
          (error, image) => {
            if (error) throw error;
            map.current.addImage("store-icon", image, { sdf: true });
            map.current.addSource("food-stores", {
              type: "vector",
        ...
        map.current.current.addSource("contours", {
          type: "vector",
