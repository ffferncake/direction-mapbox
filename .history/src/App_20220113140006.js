      ...
      map.current.on("idle", () => {
        ...
        $.ajax({
          ...
          complete: function (xhr) {
            var jo = JSON.parse(JSON.stringify(xhr.responseText));
            console.log("jo:", jo); //pass
            // var jo = xhr.responseText.toString();
            // var jo = $.parseJSON(xhr.responseText);
            ...
            directions.on("route", (event) => {
              ...
              for (const route of routes) {
                ...
                const heading = report.appendChild(
                  document.createElement("div")
                );

                // Set the class type based on clear value.
                heading.className = isClear ? "title" : "warning";
                heading.innerHTML = `${emoji} Route ${
                  route.id + 1
                } ${collision}`;

                // Add details to the individual report.
                const details = report.appendChild(
    ...
    <div>
      ...
      <div class="sidebar">
        <form onSubmit={handleSubmit}>
          ...
          <button id="zoomto" type="submit" class="btn">
            สืบค้น
          </button>
        </form>
        <h1>รายงานเส้นทาง</h1>
