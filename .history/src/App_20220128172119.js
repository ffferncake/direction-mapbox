...
export default function App() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      let hospitalGeojson = await fetch(
        "https://geoportal.rtsd.mi.th/arcgis/rest/services/OpenData/TH_CitizenInfo_Health_20200314/FeatureServer/0/query?where=1%3D1&f=geojson"
      )
      ...
      console.log(hospitalGeojson);
      return hospitalGeojson;
    };
    
    fetchData();
    // Create a popup, but don't add it to the map yet.
    const popupHospital = new mapboxgl.Popup({
      closeButton: false,
      closeOnClick: false,
      offset: 25,
    });

    map.current.on("mouseenter", "hospitalGeojson", (e) => {
      ...
      const title = e.features[0].properties.unit_name;
      ...
      popupHospital
        ...
        .setHTML(`<div id = "popup-container"><h2>${title}</h2></div>`)
        .addTo(map.current);
    });

    map.current.on("mouseleave", "hospitalGeojson", () => {
      map.current.getCanvas().style.cursor = "";
      popupHospital.remove();
    });
  ...
  return (
    <div>
      ...
      {/* </div> */}
      <div class="mapboxgl-ctrl-layers-control">
        <img
          src="https://raw.githubusercontent.com/ffferncake/InteractiveWebMap/main/hospital-pngrepo-com.png"
          alt="hospital"
        ></img>
      </div>
      {/* <!-- route report ----> */}
