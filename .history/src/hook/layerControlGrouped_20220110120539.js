...
export class layerControlGrouped {
  constructor(options) {
    ...
    if (
      (options.options && options.options.collapsed) ||
    ...
    this._layers.forEach(function (l) {
      ...
      if (l.children) {
        layersClone.forEach((child) => {
          if (child.parent && child.parent === l.id)
            l.metadata.layers = [...l.metadata.layers, child.id];
        });
      }
    });
  ...
  onAdd(map) {
    ...
    this._layers.forEach((l) => {
      ...
      if (
        ...
        !activeLayers.includes(l.parent)
      ) {
    ...
    for (let d in this._layerControlConfig) {
      ...
      this._layers.forEach((l) => {
        if (l.directory === d && !l.parent) {
          var checked = mglHelper.GetLayerVisibility(
            this._mapLayers,
            this._mapLayerIds,
            l.id
          );
      ...
      for (let g in this._layerControlConfig[d]) {
        ...
        for (let l = 0; l < groupLayers.length; l++) {
          ...
          let checked;
          checked = mglHelper.GetLayerVisibility(
            this._mapLayers,
            this._mapLayerIds,
            layer.id
          );
          ...
          // }
          let { layerSelector, newSources } = lcCreateLayerToggle(
            this._map,
            layer,
            checked,
            this._sources
    ...
    this._div.addEventListener("click", function (e) {
      ...
      if (e.target.dataset.layergroup) {
        ...
        let inputs = e.target.parentElement.querySelectorAll(
          "[data-master-layer]"
        );
...
function lcCreateLayerToggle(map, layer, checked, sources) {
  ...
  if (
    layer.metadata.lazyLoading &&
    layer.metadata.source &&
    layer.metadata.source.id &&
    layer.metadata.source.type &&
    layer.metadata.source.data
  ) {
...
function lcCheckLazyLoading(map, layer) {
  if (
    layer.dataset.lazyLoading &&
    layer.checked &&
    !layer.dataset.sourceLoaded
  ) {
    ...
    if (!source._data.features.length) {
      ...
      fetch(layer.dataset.sourceData, {
        ...
        .then((data) => {
          ...
          if (
            (newSource._data.features && !newSource._data.features.length) ||
            (newSource._data.geometries && !newSource._data.geometries.length)
          ) {
...
function buildFilter(data, layer) {
  ...
  for (var i = 0; i < fields.length; i++) {
    ...
    if (
      layer.metadata.filterSchema[fields[i]].type === "date" &&
      layer.metadata.filterSchema[fields[i]].epoch
    ) {
    ...
    switch (layer.metadata.filterSchema[fields[i]].type) {
      case "date":
        filter.push([values[i + 1], ["get", fields[i]], filterValue]);
        break;
      case "number":
        filter.push([values[i + 1], ["get", fields[i]], Number(filterValue)]);
        break;
      default:
        filter.push(["==", ["get", fields[i]], filterValue]);
    }
  }
  ...
  for (let s in schema) {
    ...
    html += `
     <div class="form-group">
       ...
       ${
         !schema[s].options
           ? `<input class="form-input" id="${s}" type="${
               schema[s].type
             }" name="${s}"  ${!schema[s].readonly ? "" : 'readonly="true"'} ${
               !schema[s].required ? "" : 'required="true"'
             }>`
       ...
       ${
         !schema[s].hint
           ? ""
           : `<p class="form-input-hint">${schema[s].hint}</p>`
       }
     </div>
     `;

    if (schema[s].type === "date" || schema[s].type === "number")
      html += `
       <div class="form-group">
