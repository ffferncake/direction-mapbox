  ...
  useEffect(() => {
    ...
    function forwardGeocoder(query) {
      var sourceObj = new mapboxgl.GeoJSONSource({
        data: '../src/data/thailand_health_facilities_th.geojson'
     });
      // var customData = fetch(
      //   "../src/data/thailand_health_facilities_th.geojson"
      // ).then((res) => res.json());
      const matchingFeatures = [];
      for (var i = 0; i < sourceObj.features.length; i++) {
        var feature = sourceObj.features[i];
        // handle queries with different capitalization than the source data by calling toLowerCase()
        if (
          feature.properties.HandlerId.toLowerCase().search(
            query.toLowerCase()
          ) !== -1
        ) {
