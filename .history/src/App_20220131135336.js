...
export default function App() {
  ...
  useEffect(() => {
    ...
    getSchool();
    const schoolUrl =
      "https://gistdaportal.gistda.or.th/arcgis/rest/services/LayerList4/FeatureServer/10/query?where=1%3D1&f=geojson";
    async function getSchool() {
      let schoolGeojson = { type: "FeatureCollection", features: [] };
      await axios.get(schoolUrl).then(function (response) {
        const school = JSON.parse(JSON.stringify(response));
        const school_data = school.data;
        console.log("response.data :::", school_data);
          for (let point of school_data) {
            let coordinate = [
